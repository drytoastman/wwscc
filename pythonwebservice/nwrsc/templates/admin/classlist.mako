...
<%
	def restrictformat(cls):
		r = cls.restrictedIndexes()
		return "indexdisallow(%s)  flagdisallow(%s)" % ((', '.join(sorted(r[0]))), (', '.join(sorted(r[1]))))
...
<td title="${restrictformat(cls)}">
	<input type="text" name="clslist-${ii}.caridxrestrict" value="${cls.caridxrestrict}" size="20"/>
</td>
