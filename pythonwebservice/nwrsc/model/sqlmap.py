...
sqlmap = {
'DELETEANNOUNCERDATA':'delete from announcer where eventid=:h1 and carid=:h2',
'DELETECAR':'delete from cars where id=:h1',
'DELETECHALLENGE':'delete from challenges where id=:h1',
'DELETECLASSGROUPMAPPING':'delete from runorder where eventid=:h1',
...
'GETDRIVER':'select * from drivers where id=:h1',
'GETDRIVERBYMEMBERSHIP':'select * from drivers where membership like :h1',
...
'GETREGISTEREDENTRANTS':'select distinct d.firstname as firstname,d.lastname as lastname,c.* from registered as x, cars as c, drivers as d where x.carid=c.carid AND c.driverid=d.driverid and x.eventid=:h1',
...
'GETRUNGROUPMAPPING':'select classcode from runorder where eventid=:h1 and rungroup=:h2 order by gorder',
'GETRUNORDERENTRANTS':'select d.firstname,d.lastname,c.*,reg.paid from drivers as d, cars as c, runorder as r LEFT JOIN registered as reg ON reg.carid=c.id and reg.eventid=r.eventid  where r.carid=c.id AND c.driverid=d.id and r.eventid=:h1 AND r.course=:h2 AND r.rungroup=:h3 order by r.row',
'GETRUNORDERROWS':'select row from runorder where eventid=:h1 AND course=:h2 AND carid=:h3',
'GETRUNORDERROWSCURRENT':'select row from runorder where eventid=:h1 AND course=:h2 AND rungroup=:h3 AND carid=:h4',
...
'HASANYRUNS':'select raw from runs where carid=:h1 limit 1',
'INSERTBLANKCHALLENGEROUND':'insert into challengerounds (challengeid,round,swappedstart,car1id,car2id) values (:h1,:h2,:h3,:h4,:h5)',
'INSERTCAR':'insert or ignore into cars (driverid,year,make,model,color,number,classcode,indexcode,tireindexed) values (:h1,:h2,:h3,:h4,:h5,:h6,:h7,:h8,:h9)',
'INSERTCHALLENGE':'insert into challenges (eventid, name, depth) values (:h1,:h2,:h3)',
'INSERTCLASSGROUPMAPPING':'insert into runorder (eventid, classcode, rungroup) values (:h1,:h2,:h3)',
...
'INSERTDRIVER':'insert or ignore into drivers (firstname,lastname,alias,email,address,city,state,zip,phone,brag,sponsor,membership) values (:h1,:h2,:h3,:h4,:h5,:h6,:h7,:h8,:h9,:h10,:h11,:h12)',
...
'REGISTERCAR':'insert or ignore into registered (eventid, carid, paid) values (:h1,:h2,:h3)',
'REGISTERCARFORCE':'insert or replace into registered (eventid, carid, paid) values (:h1,:h2,:h3)',
