...
{
	form = item.parentNode.parentNode; // todo, ugly, find form from document?
	form.carid.value = item.value;
...
function setSelectValue(select, value)
{
	...
	{
		if (select.options[ii].value == value)
		{
...
function adjustAvailableLink()
{
	...
	if (index < 0) {
		...
		link.href = "available?code=" + box.options[index].value;
	}
}
